	...
	<head>
		...
		<script>
			...
			function showListLimit(limit){
				location.href="AdminCs?listLimit=" + limit;
			}
									...
									<td>
										<c:choose>
											<c:when test="${One.cs_check eq '미확인'}">
												<button value="${One.cs_num}" class="registReply">답변 작성</button>
											</c:when>
											<c:otherwise>
												<button value="${One.cs_num}" class="viewReply">답변 보기</button>
											</c:otherwise>
										</c:choose>
									</td>
								</tr>

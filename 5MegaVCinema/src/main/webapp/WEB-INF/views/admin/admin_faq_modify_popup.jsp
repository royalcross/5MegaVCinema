...
<input type="hidden" name="FAQ_num" value="${selectedFaq.FAQ_num}" > 

<div>
	<span>카테고리</span> <br>
	<select class="category" name="FAQ_category">
	 	<option value="영화관 이용" <c:if test="${selectedFaq.FAQ_category eq '영화관 이용'}">selected</c:if>>영화관 이용</option>
	 	<option value="회원" <c:if test="${selectedFaq.FAQ_category eq '회원'}">selected</c:if>>회원</option>
	 	<option value="관람권" <c:if test="${selectedFaq.FAQ_category eq '관람권'}">selected</c:if>>관람권</option>
